module.exports = (req, res, next) => {
  try {
    ...
    const categoryId =
      parseInt(req.body.category) !== undefined
        ? parseInt(req.body.category)
        : undefined;
    const userId = req.auth.userId !== undefined ? req.auth.userId : undefined;
    const imageUrl =
      `${req.protocol}://${host}/images/${req.file.filename}` || undefined;
    ...
    return res
      .status(500)
      .json({ error: new Error("Something wrong occurred") });
  }
};
